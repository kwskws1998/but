...
from trainers.reward_trainer_general import (
...
if __name__ == "__main__":
    ...
    add_input_limit_arguments(parser)
    parser.add_argument(
    ...
    parser.add_argument(
        "--lr_scheduler_type", help="lr_scheduler_type", default="cosine_with_min_lr"
    )
    parser.add_argument("--grid_search", help="grid_search", default=False)
    parser.add_argument(
        "--gradient_acum_steps", type=int, help="gradient_acum_steps", default=8
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        choices=ET2_GAZE_CONCAT_CONDITIONS,
        default=None,
        help=(
            "ET2 GazeConcat condition: raw features, or redistributed TRT "
            "together with the other four raw gaze features"
        ),
    )
    ...
    et2_gaze_concat_condition = resolve_et2_gaze_concat_condition(
        ...
        requested_condition=args.et2_gaze_concat_condition,
        use_asym_gaussian_redistributor=use_asym_gaussian_redistributor,
    )
    if et2_gaze_concat_condition == ET2_TRT_REDISTRIBUTED_GAZE_CONCAT:
        use_asym_gaussian_redistributor = True
    elif et2_gaze_concat_condition == ET2_RAW_GAZE_CONCAT:
    ...
    if mode == "train":
        ...
        with open(folder_name_path + "/results_dataset_eval.json", "w") as f:
            json.dump(results, f, indent=4)
